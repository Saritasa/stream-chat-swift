...
struct ChannelDetailPayload<ExtraData: ExtraDataTypes>: Decodable {
  ...
  init(from decoder: Decoder) throws {
    ...
    self.config = config
    self.created = try container.decodeIfPresent(Date.self, forKey: .created) ?? config.created
    self.deleted = try container.decodeIfPresent(Date.self, forKey: .deleted)
    self.createdBy = try container.decodeIfPresent(UserEndpointPayload<ExtraData.User>.self, forKey: .createdBy)
    self.lastMessageDate = try container.decodeIfPresent(Date.self, forKey: .lastMessageDate)
    self.isFrozen = try container.decode(Bool.self, forKey: .frozen)
    self.team = try container.decodeIfPresent(String.self, forKey: .team) ?? ""
    self.extraData = try? ExtraData.Channel(from: decoder)
