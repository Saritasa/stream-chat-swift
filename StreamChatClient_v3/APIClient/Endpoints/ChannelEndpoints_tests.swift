...
class ChannelListPayloadTests: XCTestCase {
  let channelJSON: Data = {
    let url = Bundle(for: UserEndpointPayloadTests.self).url(forResource: "ChannelsQuery", withExtension: "json")!
    return try! Data(contentsOf: url)
  }()

  func test_channelQueryJSON_isSerialized_withDefaultExtraData() throws {
    let payload = try JSONDecoder.default.decode(ChannelListEndpointPayload<DefaultDataTypes>.self, from: channelJSON)
...
class ChannelEndpointPayloadTests: XCTestCase {
  let channelJSON: Data = {
    let url = Bundle(for: UserEndpointPayloadTests.self).url(forResource: "Channel", withExtension: "json")!
    return try! Data(contentsOf: url)
  }()
