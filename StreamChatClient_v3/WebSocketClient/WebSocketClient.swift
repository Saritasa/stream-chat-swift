...
class WebSocketClient {
  ...
  private func handleAppDidEnterBackground() {
    guard options.contains(.staysConnectedInBackground), connectionState.isActiveConnection else { return }
...
extension WebSocketClient: WebSocketEngineDelegate {
  ...
  func websocketDidReceiveMessage(_ message: String) {
    do {
      ...
      let webSocketError = message
        ...
        .map { ClientError.WebSocketError(error: $0?.error) }
  ...
  func websocketDidDisconnect(error engineError: WebSocketEngineError?) {
    ...
    if shouldReconnect, let reconnectionDelay = reconnectionStrategy.reconnectionDelay(forConnectionError: disconnectionError) {
      let clientError = disconnectionError.map { ClientError.WebSocketError(error: $0) }
      ...
      connectionState = .disconnected(error: disconnectionError.map { ClientError.WebSocketError(error: $0) })
    }
  }
}
