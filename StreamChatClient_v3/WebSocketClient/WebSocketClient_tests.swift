...
final class WebSocketClientTests: XCTestCase {
  ...
  func test_reconnectionStrategy_reconnectionDelayIsRequestedAndUsed() {
    ...
    let testError = WebSocketEngineError(reason: UUID().uuidString, code: 0, providerError: nil)
    ...
    AssertAsync {
      ...
      Assert.willBeEqual(
        ...
        .waitingForReconnect(error: ClientError.WebSocketError(error: testError))
      )
    }
